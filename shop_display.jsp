...
<%}else{
	%>
	<form action="Product_Search" method="post" id="search-form">
...
<form action="Product_ser" method="post">
	<div>
...
<c:forEach var="i" items ="${CardList}">
	 <div class="card-container">
			...
			<div class="instock">Left Stock :${i.stock}</div>
			</div>
			<div class="card-modify">
				<%
				if (username && userType.equals("shopkeeper")) {
				%>
				<form action="Card_send">
 ...
 <c:if test="${currentPage > 1}">
		...
		<td><a href="Product_ser?page=${currentPage - 1}">Previous</a></td>
	...
	<c:forEach begin="1" end="${noOfPages}" var="i">
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<td><a href="Product_ser?page=${i}">${i}</a></td>
	...
	<c:if test="${currentPage lt noOfPages}">
		...
		<td><a href="Product_ser?page=${currentPage + 1}">Next</a></td>
